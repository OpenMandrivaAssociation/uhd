...
# uhd firmware
Source1:	%{URL}/releases/download/v%{images_ver}/uhd-images_%{images_ver}.tar.xz
...
tar -xJf %{S:1} -C images/images --strip-components=1
...
# fix python shebangs
find . -type f -name "*.py" -exec sed -i '/^#!/ s|.*|#!%{__python}|' {} \;
...
%install
# fix python shebangs - for generated scripts
find . -type f -name "*.py" -exec sed -i '/^#!/ s|.*|#!%{__python}|' {} \;
pushd host
...
%fdupes %{buildroot}%{_docdir}/%{name}/doxygen

# test are disabled by build option.
# %%check
# pushd host
...
%{_libdir}/cmake/uhd/UHDTargets.cmake
%{_libdir}/cmake/uhd/UHDTargets-release.cmake
